...
cstackThen (("else", kelse):("if", kif):(c, kold):cstack) =  Just ((c, knew):cstack) 
    where knew = transIfElse kif kelse . kold
cstackThen (("if", kif):(c, kold):cstack) = Just ((c, knew):cstack) 
    where knew = transIfElse kif id. kold
...
cstackUntil (("begin", kloop):(c, kold):cstack) = Just ((c, knew):cstack) where knew = transWhileLoop kloop . kold
